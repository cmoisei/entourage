...
#at the moment we'll assume the default is kroll lives under titanium
KROLL_DIR = File.expand_path(ENV['KROLL_DIR'] || File.join(TITANIUM_DIR, 'kroll'))
...
#      output_file = File.join(STAGE_DIR, "#{type.to_s}-#{name.to_s}-#{rel_config[:version]}.zip")
      output_file = File.join(STAGE_DIR, "entourage-#{name.to_s}-#{rel_config[:version]}.zip")
